		...
		switch(source.Position)
			case 2 % Train a denoising CNN.
				...
				Default_Values = {['My_CNN_',datestr(datetime,'yyyymmdd_HH-MM-SS')],{'adam','sgdm','rmsprop'},Training_Params.Input_Size(1),Training_Params.Samples_Per_Image, ...
													...
													Training_Params.InitialLearnRate,{'Randomize source images','Radnomize input samples'},Training_Params.Test_Set_Ratio,{'CPU','GPU'},{'None (progress printed to command line)','Plot progress'}};
				...
				for ii=1:length(Param_Names) % For each input field.
					...
					elseif(ii == 12) % Parallelization.
						cnn_gui.(Field_Names{ii}) = uidropdown(cnn_gui.cnn_grid,'Items',Default_Values{ii},'ItemsData',{'cpu','gpu'},'Value','cpu');

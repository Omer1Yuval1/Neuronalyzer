function [Net,y,Test_Frames,Test_Classes]= NN_Index(Architecture,layers)
	...
	switch Architecture
		...
		case 'CNN_NoPooling'
			...
			XY_Cell_Cols = 1;
			% NN_Fun_Name = str2func('HiddenX3_CNN');
			[Training_Frames,Training_Classes] = Generate_DataSet(Frame_Half_Size,[],Image_MAT,XY_Cell_Cols);
			display('Training Set Successfully Generated!');
			[Test_Frames,Test_Classes] = Generate_DataSet(Frame_Half_Size,[],Image_MAT,XY_Cell_Cols);
			display('Test Set Successfully Generated!');
			if(nargin == 1)
				[Net,y] = CNN_NoPooling(Training_Frames,Training_Classes,Test_Frames,Test_Classes);
			else
				disp('V');
				[Net,y] = CNN_NoPooling(Training_Frames,Training_Classes,Test_Frames,Test_Classes,layers);
			end
	end

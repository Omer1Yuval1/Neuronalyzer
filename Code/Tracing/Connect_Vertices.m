function Data = Connect_Vertices(Im,Data,Ax)
	...
	while(1)
		...
		for s=F0 % 1:length(Data.Segments) % For each Segment.
			...
			for v=1:2 % For each end-point of segment s.
				...
				Scores = Rect_Scan_Generalized(Im,Step_Params.Rotation_Origin,Step_Params.Angle,max(Min_Scan_Width,Step_Params.Scan_Width),Step_Params.Scan_Length,Rotation_Range, ...
												Rotation_Res,Origin_Type,Im_Rows,Im_Cols,0);
				
				[Scores,Step_Params.BG_Intensity,Step_Params.BG_Peak_Width] = Normalize_Rects_Values_Generalized(Im,Scores,Step_Params.Rotation_Origin,Step_Params.Angle,Step_Params.Width,Step_Params.Scan_Length, ...
				...
				if(~isempty(Locs1)) % If a peak was detected.
					...
					if(min(XV) > 0 && min(YV) > 0 && max(XV) <= Im_Cols && max(YV) <= Im_Rows)
						InRect1 = InRect_Coordinates(Im,[XV',YV']); % Get the linear indices of the pixels within the rectangle.
				...
				if(~isempty(F1) && ~isempty(Locs1)) % If there's a collision with another segment.
					...
					if(isempty(Locs1)) % If no peaks were detected.
						...
						W = Adjust_Rect_Width_Rot_Generalized(Im,Step_Params.Rotation_Origin,Step_Params.Angle,...

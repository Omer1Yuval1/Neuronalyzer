function Reconstruct_Skeleton_Vertices(GUI_Parameters)
	
	Vertices = GUI_Parameters.Single.Workspace.Vertices;
	Rect_Width = 4;
	Rect_Length = 8;
	Parameters1 = struct();
	...
	if(size(GUI_Parameters.General.Categories_Filter_Values,1) > 0)
		for i=1:numel(Vertices)
			...
			if(length(Vertices(i).Rectangles_Angles_Skeleton) == 3 && ismember(Vertices(i).Order,GUI_Parameters.General.Categories_Filter_Values,'rows'))
				% display(i);
				for j=1:3
					% [XV,YV] = Get_Rect_Vector(O,Vertices(i).Fit_Angles_Diffs(j),Rect_Width,Rect_Length,Parameters1);
					[XV,YV] = Get_Rect_Vector(O,Vertices(i).Rectangles_Angles_Skeleton(j),Rect_Width,Rect_Length,14);
					plot(XV,YV,'Color',[.9,0,.4]);
					% fill(XV,YV,[.9,0,.4]);
				end

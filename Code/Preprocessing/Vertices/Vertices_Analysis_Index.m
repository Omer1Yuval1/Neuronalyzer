function Data = Vertices_Analysis_Index(Im,Data)
	...
	[CB_Pixels,CB_Perimeter] = Detect_Cell_Body(Im,CB_BW_Threshold,Scale_Factor,0); % Detect cell-body. Data.Image0
	% [CB_Vertices,Pixels0,Pixels1] = Find_CB_Vertices(Im,CB_Perimeter,CB_Pixels,Scale_Factor,CB_BW_Threshold,0);
	...
	[Y,X] = ind2sub(size(Im),CB_Pixels);
	Data.CB(1).Center = [mean(X),mean(Y)];
	Data.CB.Area = length(CB_Pixels)*Scale_Factor;
	Data.CB.Mean_Intensity = mean(Im(CB_Pixels));
	% Data.CB.Vertices = CB_Vertices;
	% [CBy,CBx] = ind2sub(size(Im1),CB_Pixels);
	% return;
	
	[Im1_NoiseReduction,Im1_branchpoints,Im1_endpoints,Im_Skel_Rad] = Pixel_Trace_Post_Proccessing(Data.Info.Files(1).Binary_Image,Scale_Factor); % Skeletonization. Data.Im_BW
	[Data.Vertices,Data.Segments] = Segment_Skeleton(Im1_NoiseReduction,Im1_branchpoints,Im1_endpoints,Im_Skel_Rad);
	
    Data = Match_Vertex_Rects_To_Segments_Skel(Data); % Here only the size of the image is used, so no need to send the enhanced image as an input argument.

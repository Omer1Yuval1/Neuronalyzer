function Struct1 = Menorahs_Receptive_Field_Length_Struct(Workspace1,Field_Name)
	...
	for m=1:numel(Workspace1.Menorahs)
		...
		D = abs(Workspace1.Menorahs(m).Max_Anterior - Workspace1.Menorahs(m).Max_Posterior);
		if(length(Fb) > 0 && D > 0) % If it has at least one branch of order >= 3.
			
			Struct1(end+1).Property = Workspace1.Menorahs(m).Total_Length / (D ... % The total length of the menorah,

function Struct1 = Total_Length_Per_Menorah(Workspace1)
	...
	for m=1:max([Workspace1.Branches.Menorah]) % For each Menorah index.
		...
		if(length(Fb) > 0)
			Struct1(end+1).Property = sum([Workspace1.Branches(find([Workspace1.Branches.Menorah] == m)).Length]); % Number of branches of menorah 'm'. x-axis.
			Struct1(end).Order = 0;

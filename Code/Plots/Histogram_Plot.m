function Histogram_Plot(Input_Struct,GUI_Parameters,Visuals,X_Min_Max,BinSize,XLabel,YLabel,Title1)
	...
	if(Groups_Num > 0) % If at least one group is ON.
		...
		if(Ncat == 0)
			...
			for i=1:Groups_Num % For each group (a unique combination of selected features).
				...
				Legend_Handles_Array(i) = histogram(Input_Struct(i).XValues,X_Min_Max(1):BinSize:X_Min_Max(2),'Normalization','probability'); % No normalization.

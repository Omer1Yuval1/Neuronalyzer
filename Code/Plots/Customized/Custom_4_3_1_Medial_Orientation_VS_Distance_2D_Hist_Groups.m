function Custom_4_3_1_Medial_Orientation_VS_Distance_2D_Hist_Groups(GUI_Parameters,Visuals,YLabel,Title1)
	...
	for g=1:size(Groups,2)
		...
		for w=1:length(Fg) % For each neuron (=animal).
			...
			for s=1:numel(W.Segments)
				...
				if(g == 1)
					V1 = [V1 , [Angle_Diffs(f2) ; Rects_Distances(f2)]];
				elseif(g == 2)
					V2 = [V2 , [Angle_Diffs(f2) ; Rects_Distances(f2)]];
				end
			end
	...
	subplot(2,1,1);
		histogram2(V1(1,:),V1(2,:),'Normalization','probability','FaceColor','flat');
		...
		title('WT-Crowded','FontSize',22,'Color',Visuals.Active_Colormap(1,:));
	...
	subplot(2,1,2);
		histogram2(V2(1,:),V2(2,:),'Normalization','probability','FaceColor','flat');
		...
		set(gca,'YColor',Visuals.Active_Colormap(1,:));
		title('WT-Isolated','FontSize',22,'Color',Visuals.Active_Colormap(1,:));
		% xlim([0,pi./2]); % xlim([0,pi]);
		xlim([0,180]);
		ylim([0,45]);
		grid on;

function Plot_Angles_Histogram_MonteCarlo(X,Y,GUI_Parameters,BinSize)
	...
	V1 = [0,120,240 ; 0,90,180 ; 0,60,120]; % Morphologies.
	V2 = 15:25; % Noise range.
	I = [];
	Vp = 0;
	for i=1:size(V1,1)
		for j=V2
			[f_monte,xi_monte,a_monte] = randomJunction(V1(i,:),j,numel(X)); % numel(X)*1. 100000.
	...
	title(['Angles: ',num2str(V1(I(1),:)) , ' ; Noise: ',num2str(I(2))]);

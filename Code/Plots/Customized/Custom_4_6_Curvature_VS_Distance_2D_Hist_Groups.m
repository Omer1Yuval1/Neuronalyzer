function Custom_4_6_Curvature_VS_Distance_2D_Hist_Groups(GUI_Parameters,Visuals,YLabel,Title1)
	...
	V1 = zeros(2,10^6); % [Curvature ; Medial Distance].
	V2 = zeros(2,10^6); % ".
    I1 = [1,0];
    I2 = [1,0];
	for g=1:size(Groups,2) % For each group.
		
		Fg = find([GUI_Parameters.Workspace.Grouping] == Groups(1,g) & [GUI_Parameters.Workspace.Genotype] == Groups(2,g));
		
		for w=1:length(Fg) % For each neuron (=animal) within group g.
			...
			if(~isempty(W.Medial_Axis))
				...
				if(g == 1)
					Li = length(Vc_Dist_0);
					I1(2) = I1(1) + Li - 1;
					V1(:,I1(1):I1(2)) = [Vc_Dist_0' ; Dist_Vector_0'];
					I1(1) = I1(2) + 1;
				elseif(g == 2)
					Li = length(Vc_Dist_0);
					I2(2) = I2(1) + Li - 1;
					V2(:,I2(1):I2(2)) = [Vc_Dist_0' ; Dist_Vector_0'];
					I2(1) = I2(2) + 1;
				end
			end			
		end
	...
	subplot(2,1,1);
		histogram2(V1(1,:),V1(2,:),'Normalization','pdf','FaceColor','flat');
	...
	subplot(2,1,2);
		histogram2(V2(1,:),V2(2,:),'Normalization','pdf','FaceColor','flat');

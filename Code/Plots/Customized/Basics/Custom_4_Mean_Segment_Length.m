function Custom_4_Mean_Segment_Length(GUI_Parameters,Visuals,YLabel,Title1)
	...
	for g=1:size(Groups,2)
		...
		Mean1 = nanmean(V1);
		STD_SE = nanstd(V1); % STD.
		C = [0,0,0];
		hold on;
		scatter(g*ones(1,length(V1)),V1,5,'MarkerFaceColor',[.5,.5,.5],'MarkerEdgeColor',ColorMap(g,:),'jitter','on','jitterAmount',.2);

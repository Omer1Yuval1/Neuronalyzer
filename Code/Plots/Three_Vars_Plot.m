function Three_Vars_Plot(Input_Struct,GUI_Parameters,Visuals,XLabel,YLabel,Z_Label,Title1)
	...
	if(Groups_Num > 0) % If at least one group is ON.
		...
		if(Ncat == 0)
			...
			for i=1:Groups_Num % For each group (a unique combination of selected features).
				...
				hold on;
				scatter3(Input_Struct(i).XValues,Input_Struct(i).YValues,Input_Struct(i).ZValues,Visuals.Scatter_Dot_Size1,'MarkerFaceColor',Input_Struct(i).Color,'MarkerEdgeColor',Input_Struct(i).Color,'jitter','on','jitterAmount',Visuals.Jitter1);
			...
			zlabel(Z_Label,'FontSize',Visuals.Axes_Titles_Font_Size);
